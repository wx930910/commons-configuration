...
public class TestNodeTreeWalker {
	public static ConfigurationNodeVisitor<ImmutableNode> mockConfigurationNodeVisitor1() {
		...
		doAnswer((stubInvo) -> {
			...
			return null;
		}).when(mockInstance).visitAfterChildren(any(ImmutableNode.class), any(NodeHandler.class));
		doAnswer((stubInvo) -> {
			ImmutableNode node = stubInvo.getArgument(0);
			NodeHandler<ImmutableNode> handler = stubInvo.getArgument(1);
			mockFieldVariableVisitedNodes.add(handler.nodeName(node));
			return null;
		}).when(mockInstance).visitBeforeChildren(any(ImmutableNode.class), any(NodeHandler.class));
	...
	public void testWalkDFS() {
		...
		doAnswer((stubInvo) -> {
			...
			return null;
		}).when(visitor).visitAfterChildren(any(ImmutableNode.class), any(NodeHandler.class));
		doAnswer((stubInvo) -> {
			ImmutableNode node = stubInvo.getArgument(0);
			NodeHandler<ImmutableNode> handler = stubInvo.getArgument(1);
			visitorVisitedNodes.add(handler.nodeName(node));
			return null;
		}).when(visitor).visitBeforeChildren(any(ImmutableNode.class), any(NodeHandler.class));
