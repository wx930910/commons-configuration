...
public class TestCompositeConfiguration {
	...
	/**
	 * Tests adding values. Make sure they _DON'T_ override any other properties but
	 * add to the existing properties and keep sequence
	 */
	@Test
	...
	public void testGettingSubset() throws Exception {
		...
		assertEquals("Make sure the initial loaded configs subset overrides any later add configs subset", "1",
				subset.getString("short"));

		cc.setProperty("test.short", "43");
		subset = cc.subset("test");
		assertEquals("Make sure the initial loaded configs subset overrides any later add configs subset", "43",
				subset.getString("short"));
	}
	...
	public void testClone() {
		...
		assertEquals("Wrong number of contained configurations", cc.getNumberOfConfigurations(),
				cc2.getNumberOfConfigurations());

		final StrictConfigurationComparator comp = new StrictConfigurationComparator();
		for (int i = 0; i < cc.getNumberOfConfigurations(); i++) {
			assertEquals("Wrong configuration class at " + i, cc.getConfiguration(i).getClass(),
					cc2.getConfiguration(i).getClass());
			assertNotSame("Configuration was not cloned", cc.getConfiguration(i), cc2.getConfiguration(i));
			assertTrue("Configurations at " + i + " not equal",
					comp.compare(cc.getConfiguration(i), cc2.getConfiguration(i)));
		}
	...
	/**
	 * Tests cloning if one of the contained configurations does not support this
	 * operation. This should cause an exception.
	 */
	@Test(expected = ConfigurationRuntimeException.class)
	public void testCloneNotSupported() {
		cc.addConfiguration(NonCloneableConfiguration.mockAbstractConfiguration1());
		cc.clone();
	}
	...
	/**
	 * Prepares a test for interpolation with multiple configurations and similar
	 * properties.
	 */
	 ...
	 * This test is related to CONFIGURATION-441.
	 */
	@Test
	...
	/**
	 * Tests whether interpolation works if a variable references a property with
	 * multiple values. This test is related to CONFIGURATION-632.
	 */
	@Test
	...
	/**
	 * Tests the behavior of setListDelimiterHandler() if the in-memory
	 * configuration is not derived from BaseConfiguration. This test is related to
	 * CONFIGURATION-476.
	 */
	@Test
	...
	/**
	 * Tests whether a configuration can act as both regular child configuration and
	 * in-memory configuration. This test is related to CONFIGURATION-471.
	 */
	@Test
