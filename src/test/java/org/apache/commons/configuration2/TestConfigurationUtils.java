...
public class TestConfigurationUtils {
	...
	public void testToString() {
		...
		assertEquals("String representation of a configuration", "one=1" + lineSeparator + "two=2",
		...
		config.setProperty("one", "1");
		assertEquals("String representation of a configuration", "two=2" + lineSeparator + "one=1",
				ConfigurationUtils.toString(config));
	}
	...
	public void testConvertToHierarchical() {
		...
		final BaseHierarchicalConfiguration hc = (BaseHierarchicalConfiguration) ConfigurationUtils
	...
	/**
	 * Tests converting an already hierarchical configuration using an expression
	 * engine. The new engine should be set.
	 */
	@Test
	 ...
	 * expression engine. In this case the expression engine of the configuration
	...
	/**
	 * Tests that the structure of the resulting hierarchical configuration does not
	 * depend on the order of properties in the source configuration. This test is
	...
	public void testConvertToHierarchicalOrderOfProperties() {
		...
		final HierarchicalConfiguration<ImmutableNode> hc = (HierarchicalConfiguration<ImmutableNode>) ConfigurationUtils
	...
	public void testCloneConfigurationNotSupported() {
		final AbstractConfiguration myNonCloneableConfig = NonCloneableConfiguration.mockAbstractConfiguration1();
	...
	public void testEnableRuntimeExceptions() {
		final PropertiesConfiguration config = new PropertiesConfiguration() {
			...
			protected void addPropertyDirect(final String key, final Object value) {
				...
				fireError(ConfigurationErrorEvent.WRITE, ConfigurationEvent.ADD_PROPERTY, key, value,
	...
	/**
	 * Tries to enable runtime exceptions for a configuration that does not inherit
	 * from EventSource. This should cause an exception.
	 */
	...
	private static class CloneableSynchronizer extends NonCloneableSynchronizer implements Cloneable {
		...
		/**
		 * Creates a new instance of {@code CloneableSynchronizer} and sets the clone
		 * flag.
		 *
