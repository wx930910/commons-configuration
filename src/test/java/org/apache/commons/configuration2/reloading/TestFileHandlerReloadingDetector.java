...
public class TestFileHandlerReloadingDetector {
	public static FileHandlerReloadingDetector mockFileHandlerReloadingDetector1(final File file) {
		File mockFieldVariableMockFile;
		FileHandlerReloadingDetector mockInstance = spy(new FileHandlerReloadingDetector(null, 0));
		mockFieldVariableMockFile = file;
		doAnswer((stubInvo) -> {
			return mockFieldVariableMockFile;
		}).when(mockInstance).getFile();
		return mockInstance;
	}

	public static FileHandlerReloadingDetector mockFileHandlerReloadingDetector2(final File file, final long delay) {
		File mockFieldVariableMockFile;
		FileHandlerReloadingDetector mockInstance = spy(new FileHandlerReloadingDetector(null, delay));
		mockFieldVariableMockFile = file;
		doAnswer((stubInvo) -> {
			return mockFieldVariableMockFile;
		}).when(mockInstance).getFile();
	...
	public void testIsReloadingRequiredTrue() throws Exception {
		...
		EasyMock.replay(f);
		final FileHandlerReloadingDetector detector = TestFileHandlerReloadingDetector
				.mockFileHandlerReloadingDetector1(f);
	...
	/**
	 * Tests a refresh cycle with a detected reload operation and a notification
	 * that reloading was performed.
	 */
	@Test
	...
	public void testRefreshDelay() throws Exception {
		...
		EasyMock.replay(f);
		final FileHandlerReloadingDetector detector = TestFileHandlerReloadingDetector
				.mockFileHandlerReloadingDetector2(f, 60 * 60 * 1000L);
		detector.reloadingPerformed();
	...
	public void testGetFileJarURL() throws Exception {
		...
		final URL url = new URL(
				"jar:" + new File("conf/resources.jar").getAbsoluteFile().toURI().toURL() + "!/test-jar.xml");
