...
public class TestDefaultParametersManager {
	public static DefaultParametersHandler<FileBasedBuilderParameters> mockDefaultParametersHandler1() {
		DefaultParametersHandler<FileBasedBuilderParameters> mockInstance = mock(DefaultParametersHandler.class);
		doAnswer((stubInvo) -> {
			FileBasedBuilderParameters parameters = stubInvo.getArgument(0);
			parameters.setThrowExceptionOnMissing(true).setEncoding(DEF_ENCODING).setListDelimiterHandler(listHandler);
			return null;
		}).when(mockInstance).initializeDefaults(any(FileBasedBuilderParameters.class));
		return mockInstance;
	}
	...
	public void testRegisterDefaultsHandlerNoClass() {
		manager.registerDefaultsHandler(null, TestDefaultParametersManager.mockDefaultParametersHandler1());
	}
	...
	public void testApplyDefaults() {
		manager.registerDefaultsHandler(FileBasedBuilderParameters.class,
				TestDefaultParametersManager.mockDefaultParametersHandler1());
	...
	/**
	 * Tests whether default values are also applied when a sub parameters class is
	 * created.
	 */
	@Test
	...
	public void testApplyDefaultsStartClass() {
		manager.registerDefaultsHandler(FileBasedBuilderParameters.class,
				TestDefaultParametersManager.mockDefaultParametersHandler1(), XMLBuilderParameters.class);
	...
	/**
	 * Tests whether multiple handlers can be registered for the same classes and
	 * whether they are called in the correct order.
	 */
	@Test
	public void testApplyDefaultsMultipleHandlers() {
		...
		manager.registerDefaultsHandler(XMLBuilderParameters.class,
				parameters -> parameters.setThrowExceptionOnMissing(false)
						.setListDelimiterHandler(EasyMock.createMock(ListDelimiterHandler.class))
	...
	/**
	 * Tests whether initializeParameters() ignores null input. (We can only test
	...
	public void testUnregisterDefaultsHandlerAll() {
		final DefaultParametersHandler<FileBasedBuilderParameters> handler = TestDefaultParametersManager
				.mockDefaultParametersHandler1();
