...
public class TestReloadingCombinedConfigurationBuilderFileBased {
	public static BasicConfigurationBuilder<BaseHierarchicalConfiguration> mockBasicConfigurationBuilder1(
			final BaseHierarchicalConfiguration conf) throws ConfigurationException {
	...
	/**
	 * Helper method for writing a test file for reloading. The file will be created
	 * in the test directory. It is also scheduled for automatic deletion after the
	 * test.
	 *
	 ...
	 */
	@Test
	public void testReloadFromFile() throws ConfigurationException, IOException, ConfigurationException {
		final File xmlConf1 = writeReloadFile(null, 1, 0);
		final File xmlConf2 = writeReloadFile(null, 2, 0);
		final ReloadingDetectorFactory detectorFactory = (handler, params) -> AlwaysReloadingDetector
				.mockReloadingDetector1();
		...
		builder.configure(parameters.combined()
				.setDefinitionBuilder(
						TestReloadingCombinedConfigurationBuilderFileBased.mockBasicConfigurationBuilder1(defConf))
				.registerChildDefaultsHandler(FileBasedBuilderProperties.class, new CopyObjectDefaultHandler(
						new FileBasedBuilderParametersImpl().setReloadingDetectorFactory(detectorFactory))));
	 ...
	 */
	@Test
	public void testConcurrentGetAndReload() throws Exception, ConfigurationException {
		final int threadCount = 4;
		final int loopCount = 100;
		final ReloadingDetectorFactory detectorFactory = (handler, params) -> RandomReloadingDetector
				.mockReloadingDetector1();
		final BaseHierarchicalConfiguration defConf = new BaseHierarchicalConfiguration();
		...
		builder.configure(parameters.combined()
				...
				.registerChildDefaultsHandler(BasicBuilderProperties.class,
						new CopyObjectDefaultHandler(new BasicBuilderParameters().setSynchronizer(sync)))
				.registerChildDefaultsHandler(FileBasedBuilderProperties.class, new CopyObjectDefaultHandler(
						new FileBasedBuilderParametersImpl().setReloadingDetectorFactory(detectorFactory))));

		assertEquals("Wrong initial value", "100",
	 ...
	 */
	private void checkReloadDefinitionFile(final File defFile)
	...
	/**
	 * Tests whether a change in the definition file is detected and causes a reload
	 * if a specific builder for the definition configuration is provided.
	 */
	@Test
	public void testReloadDefinitionFileExplicitBuilder()
		...
		builder.configure(parameters.combined()
				.setDefinitionBuilder(new ReloadingFileBasedConfigurationBuilder<>(XMLConfiguration.class)
						.configure(parameters.xml().setReloadingRefreshDelay(0L).setFile(defFile))));
		checkReloadDefinitionFile(defFile);
	}

	/**
	 * Tests whether the default definition builder is capable of detecting a change
	 * in the definition configuration.
	 */
	@Test
	public void testReloadDefinitionFileDefaultBuilder()
		...
		final File defFile = folder.newFile();
		builder.configure(parameters.combined()
				.setDefinitionBuilderParameters(parameters.xml().setReloadingRefreshDelay(0L).setFile(defFile)));
		checkReloadDefinitionFile(defFile);
	}
	...
	private static class ReloadThread extends Thread {
		...
		ReloadThread(final ReloadingCombinedConfigurationBuilder bldr, final int[] failures, final int index,
				final int count) {
			builder = bldr;
