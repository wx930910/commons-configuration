...
public class TestCombinedConfigurationBuilder {
	public static BasicConfigurationBuilder<BaseHierarchicalConfiguration> mockBasicConfigurationBuilder1(
			final BaseHierarchicalConfiguration conf) {
		BaseHierarchicalConfiguration mockFieldVariableConfiguration;
		BasicConfigurationBuilder<BaseHierarchicalConfiguration> mockInstance = spy(
				new BasicConfigurationBuilder(BaseHierarchicalConfiguration.class));
		mockFieldVariableConfiguration = conf;
		try {
			doAnswer((stubInvo) -> {
				return mockFieldVariableConfiguration;
			}).when(mockInstance).getConfiguration();
	...
	/**
	 * Creates a configuration builder for the definition configuration which always
	 * returns the passed in definition configuration.
	 *
	...
	protected static BasicConfigurationBuilder<? extends BaseHierarchicalConfiguration> createDefinitionBuilder(
		...
		return TestCombinedConfigurationBuilder.mockBasicConfigurationBuilder1(defConfig);
	}

	/**
	 * Convenience method for creating a definition configuration. This method
	 * creates a configuration containing a tag referring to a configuration source.
	 * The tag has attributes defined by the given map.
	 *
	...
	/**
	 * Tests loading a definition file with optional and non optional configuration
	 * sources. One non optional does not exist, so this should cause an exception.
	 */
	@Test(expected = ConfigurationException.class)
	...
	public void testLoadOptionalForceCreate() throws ConfigurationException {
		...
		final BasicConfigurationBuilder<? extends BaseHierarchicalConfiguration> defBuilder = createDefinitionBuilder(
				defConfig);
	...
	/**
	 * Tests the behavior of builderNames() before the result configuration has been
	 * created.
	 */
	@Test
	...
	/**
	 * Prepares a test with a combined configuration that uses a single sub builder.
	 * This method adds some default attributes to the given map, creates the
	 * corresponding definition builder and configures the combined builder.
	 *
	...
	private BasicConfigurationBuilder<? extends HierarchicalConfiguration<ImmutableNode>> prepareSubBuilderTest(
		...
		final BaseHierarchicalConfiguration defConfig = createDefinitionConfig("xml", attrs);
		final BasicConfigurationBuilder<? extends HierarchicalConfiguration<ImmutableNode>> defBuilder = createDefinitionBuilder(
				defConfig);
		builder.configure(new CombinedBuilderParametersImpl().setDefinitionBuilder(defBuilder));
		return defBuilder;
	}
	...
	/**
	 * Tests a reset of the builder. The configuration instance should be created
	 * anew.
	 */
	@Test
	public void testResetBuilder() throws ConfigurationException {
		final Map<String, Object> attrs = new HashMap<>();
		final BasicConfigurationBuilder<? extends HierarchicalConfiguration<ImmutableNode>> defBuilder = prepareSubBuilderTest(
				attrs);
		final CombinedConfiguration cc = builder.getConfiguration();
	...
	public void testReactOnSubBuilderChange() throws ConfigurationException {
		...
		prepareSubBuilderTest(attrs);
		final CombinedConfiguration cc = builder.getConfiguration();
		final BasicConfigurationBuilder<?> subBuilder = (BasicConfigurationBuilder<?>) builder
	...
	public void testRemoveSubBuilderListener() throws ConfigurationException {
		...
		prepareSubBuilderTest(attrs);
		final CombinedConfiguration cc = builder.getConfiguration();
		final BasicConfigurationBuilder<?> subBuilder2 = (BasicConfigurationBuilder<?>) builder
				.getNamedBuilder(BUILDER_NAME);
	...
	/**
	 * Helper method for testing the attributes of a combined configuration created
	 * by the builder.
	 *
	...
	/**
	 * Tests whether attributes are correctly set on the combined configurations for
	 * the override and additional sections.
	 */
	@Test
	public void testCombinedConfigurationAttributes() throws ConfigurationException {
		...
		final CombinedConfiguration cc2 = (CombinedConfiguration) cc
	...
	public void testCombinedConfigurationNoAdditional() throws ConfigurationException {
		builder.configure(createParameters().setFile(TEST_FILE));
		final CombinedConfiguration cc = builder.getConfiguration();
		assertNull("Additional configuration was found",
	...
	public void testCombinedConfigurationListNodes() throws ConfigurationException {
		...
		final CombinedConfiguration cca = (CombinedConfiguration) cc
				.getConfiguration(CombinedConfigurationBuilder.ADDITIONAL_NAME);
	...
	public void testCustomBuilderProvider() throws ConfigurationException {
		...
		builder.configure(new CombinedBuilderParametersImpl()
				.setDefinitionBuilder(createDefinitionBuilder(createDefinitionConfig(tagName, attrs))).registerProvider(
						tagName, decl -> TestCombinedConfigurationBuilder.mockBasicConfigurationBuilder1(dataConf)));
		final CombinedConfiguration cc = builder.getConfiguration();
	 ...
	 * configuration definition file and that system properties can be added to the
	...
	public void testJndiConfiguration() throws ConfigurationException {
		...
		builder.configure(new CombinedBuilderParametersImpl()
				.setDefinitionBuilderParameters(createParameters().setFile(multiFile)));
		final CombinedConfiguration cc = builder.getConfiguration();
	...
	public void testCustomEntityResolver() throws ConfigurationException {
		...
		final EntityResolverWithPropertiesTestImpl resolver = (EntityResolverWithPropertiesTestImpl) xmlConf
				.getEntityResolver();
	...
	public void testConfigureEntityResolverWithProperties() throws ConfigurationException {
		...
		final EntityResolverWithPropertiesTestImpl resolver = (EntityResolverWithPropertiesTestImpl) xmlParams
				.getEntityResolver();
	...
	/**
	 * Helper method for testing whether the file system can be customized in the
	...
	/**
	 * Tests whether a default base path for all file-based child configurations can
	 * be set in the builder parameters.
	 */
	@Test
	public void testDefaultBasePathInParameters() throws ConfigurationException {
		...
		builder.configure(new CombinedBuilderParametersImpl().setBasePath(basePath)
				.setDefinitionBuilderParameters(createParameters().setFile(testFile)));
		builder.getConfiguration();
		final XMLBuilderParametersImpl xmlParams = new XMLBuilderParametersImpl();
	...
	/**
	 * Tests whether the default base path for file-based configurations is derived
	 * from the configuration definition builder.
	 */
	@Test
	public void testDefaultBasePathFromDefinitionBuilder() throws ConfigurationException, IOException {
		...
		builder.configure(new CombinedBuilderParametersImpl().setDefinitionBuilderParameters(
				createParameters().setBasePath(ConfigurationAssert.TEST_DIR.getAbsolutePath()).setFileName(testFile)));
		builder.getConfiguration();
		final XMLBuilderParametersImpl xmlParams = new XMLBuilderParametersImpl();
		builder.initChildBuilderParameters(xmlParams);
	...
	public void testBasePathForChildConfigurations() throws ConfigurationException {
		...
		builder.configure(new CombinedBuilderParametersImpl().setBasePath(deepDir.getAbsolutePath())
				.setDefinitionBuilder(TestCombinedConfigurationBuilder.mockBasicConfigurationBuilder1(defConfig)));
	...
	public void testCustomResultConfiguration() throws ConfigurationException {
		...
		builder.configure(new CombinedBuilderParametersImpl()
				.setDefinitionBuilderParameters(new XMLBuilderParametersImpl().setFile(testFile))
	...
	/**
	 * Tests whether a child configuration builder inherits the event listeners from
	 * its parent.
	 */
	@Test
	public void testConfigurationBuilderProviderInheritEventListeners() throws ConfigurationException {
		...
		final CombinedConfiguration cc = builder.getConfiguration();
		final CombinedConfiguration cc2 = (CombinedConfiguration) cc.getConfiguration("subcc");
		final Collection<EventListener<? super ConfigurationEvent>> listeners = cc2
	...
	/**
	 * Tests whether the base path can be inherited to child combined configuration
	 * builders.
	 */
	@Test
	public void testConfigurationBuilderProviderInheritBasePath() throws ConfigurationException {
		...
		final String basePath = ConfigurationAssert.OUT_DIR.getAbsolutePath();
		builder.configure(new CombinedBuilderParametersImpl().setBasePath(basePath)
				.setDefinitionBuilderParameters(createParameters().setFile(envFile)));
		builder.getConfiguration();
	...
	public void testInitChildBuilderParametersDefaultChildProperties() throws ConfigurationException {
		...
		builder.configure(
				parameters.combined().setDefinitionBuilderParameters(parameters.fileBased().setFile(TEST_FILE))
						.registerChildDefaultsHandler(FileBasedBuilderProperties.class,
								new CopyObjectDefaultHandler(new FileBasedBuilderParametersImpl()
										.setReloadingRefreshDelay(defRefresh).setThrowExceptionOnMissing(true)))
						.registerChildDefaultsHandler(XMLBuilderProperties.class,
								new CopyObjectDefaultHandler(new XMLBuilderParametersImpl().setValidating(false)
										.setExpressionEngine(new XPathExpressionEngine())
										.setReloadingRefreshDelay(xmlRefresh))));
		builder.getConfiguration();
		final XMLBuilderParametersImpl params = new XMLBuilderParametersImpl();
		builder.initChildBuilderParameters(params);
		assertTrue("Wrong expression engine",
	...
	public void testInterpolationOverMultipleSources() throws ConfigurationException {
		...
		final HierarchicalConfiguration<ImmutableNode> subConfig = xmlConfig
	...
	/**
	 * Tests that child configuration builders are not initialized multiple times.
	 * This test is releated to CONFIGURATION-687.
	 */
	@Test
	...
	/**
	 * Loads a test file which includes a MultiFileConfigurationBuilder declaration
	...
	/**
	 * Tests whether a configuration created by a MultiFileConfigurationBuilder has
	...
	public void testMultiTenentConfigurationProperties() throws ConfigurationException {
		...
		final HierarchicalConfiguration<?> multiConf = (HierarchicalConfiguration<?>) config
				.getConfiguration("clientConfig");
		assertTrue("Expression engine not configured",
	...
	/**
	 * Helper method for testing whether properties of a MultiFileConfiguration can
	 * be obtained.
	 *
	...
	public void testMultiTenentConfigurationReloading() throws ConfigurationException, InterruptedException {
		...
		try {
			...
			final BasicConfigurationBuilder<? extends Configuration> multiBuilder = (BasicConfigurationBuilder<? extends Configuration>) event
					.getSource();
	...
	/**
	 * Tests that the combined configuration has been fully constructed (including
	 * its root node) when it is returned from the builder.
	 */
	@Test
	...
	private static XMLBuilderParameters prepareParamsForInheritanceTest(final Parameters params) {
		final DefaultExpressionEngineSymbols symbols = new DefaultExpressionEngineSymbols.Builder(
				DefaultExpressionEngineSymbols.DEFAULT_SYMBOLS).setPropertyDelimiter("/").create();
		...
		return params.xml().setExpressionEngine(engine).setListDelimiterHandler(listDelimiterHandler)
				.setFile(TEST_FILE);
	}
	...
	/**
	 * A test builder provider implementation for testing whether providers can be
	...
	public static class BuilderProviderTestImpl implements ConfigurationBuilderProvider {
		...
		public ConfigurationBuilder<? extends Configuration> getConfigurationBuilder(
			...
			return TestCombinedConfigurationBuilder.mockBasicConfigurationBuilder1(config);
		}
	}

	/**
	 * A specialized entity resolver implementation for testing whether properties
	 * of a catalog resolver are correctly set.
	 */
	...
	/**
	 * A test file system implementation for testing whether a custom file system
	 * class can be specified in the configuration definition file.
	 */
	...
	/**
	 * A custom Lookup implementation for testing whether lookups can be defined in
	 * the definition configuration. This lookup supports some variables referencing
	 * test files.
	 */
