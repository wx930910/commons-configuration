...
public class TestReloadingFileBasedConfigurationBuilder {
	...
	public void testCreateReloadingDetectorDefaultFactory() throws ConfigurationException {
		...
		final FileHandlerReloadingDetector detector = (FileHandlerReloadingDetector) builder
				.createReloadingDetector(handler, params);
	 ...
	 */
	@Test
	public void testReloadingDetectorIsReloadingRequired() throws ConfigurationException, Exception {
		final ReloadingDetector detector = EasyMock.createMock(ReloadingDetector.class);
		...
		final ReloadingFileBasedConfigurationBuilder<PropertiesConfiguration> builder = spy(
				new ReloadingFileBasedConfigurationBuilder(PropertiesConfiguration.class));
		ReloadingDetector builderMockDetector;
		FileHandler[] builderHandlerForDetector = new FileHandler[1];
		builderMockDetector = detector;
		doAnswer((stubInvo) -> {
			FileHandler handler = stubInvo.getArgument(0);
			builderHandlerForDetector[0] = handler;
			return builderMockDetector;
		}).when(builder).createReloadingDetector(any(), any());
		...
		assertSame("Wrong file handler", builder.getFileHandler(), builderHandlerForDetector[0]);
		EasyMock.verify(detector);
	}

	/**
	 * Tests the reloadingPerformed() implementation of the detector associated with
	 * the reloading controller.
	 */
	@Test
	public void testReloadingDetectorReloadingPerformed() throws ConfigurationException, Exception {
		final ReloadingDetector detector = EasyMock.createMock(ReloadingDetector.class);
	 ...
	 */
	@Test
	public void testResetReloadingStateInGetConfiguration() throws ConfigurationException, Exception {
		final ReloadingDetector detector = EasyMock.createMock(ReloadingDetector.class);
		EasyMock.expect(detector.isReloadingRequired()).andReturn(Boolean.TRUE);
		detector.reloadingPerformed();
		EasyMock.replay(detector);
	 ...
	 */
	@Test
	public void testReloadingControllerEvents() throws ConfigurationException, Exception {
		final ReloadingDetector detector = EasyMock.createMock(ReloadingDetector.class);
		EasyMock.expect(detector.isReloadingRequired()).andReturn(Boolean.TRUE);
		final ReloadingFileBasedConfigurationBuilder<PropertiesConfiguration> builder = spy(
				new ReloadingFileBasedConfigurationBuilder(PropertiesConfiguration.class));
