...
public class TestPropertiesConfigurationLayout {
	PropertiesBuilder configBuilder = null;
	...
	public void setUp() throws Exception, ConfigurationException {
		config = spy(PropertiesConfiguration.class);
		doAnswer((stubInvo) -> {
			...
			if (PropertiesConfiguration.getInclude().equals(key)) {
				config.getLayout().load(config, configBuilder.getReader());
				return false;
			}
			return true;
		}).when(config).propertyLoaded(any(), any(), any());
	...
	public void testInit() {
		...
		final Iterator<EventListener<? super ConfigurationEvent>> it = config.getEventListeners(ConfigurationEvent.ANY)
				.iterator();
	...
	public void testReadSimple() throws ConfigurationException {
		...
		assertEquals("Wrong number of blanc lines", 0, layout.getBlancLinesBefore(TEST_KEY));
	...
	public void testBlancLines() throws ConfigurationException {
		...
		builder.addProperty(TEST_KEY, TEST_VALUE);
		layout.load(config, builder.getReader());
		assertEquals("Wrong number of blanc lines", 2, layout.getBlancLinesBefore(TEST_KEY));
	...
	public void testCombineComments() throws ConfigurationException {
		...
		builder.addProperty(TEST_KEY, TEST_VALUE + "2");
		layout.load(config, builder.getReader());
		assertEquals("Wrong combined comment", TEST_COMMENT + CRNORM + TEST_COMMENT,
				layout.getCanonicalComment(TEST_KEY, false));
		assertEquals("Wrong combined blanc numbers", 0, layout.getBlancLinesBefore(TEST_KEY));
	}
	...
	public void testHeaderCommentWithBlancs() throws ConfigurationException {
		...
		builder.addProperty(TEST_KEY, TEST_VALUE);
		layout.load(config, builder.getReader());
		assertEquals("Wrong header comment", TEST_COMMENT + CRNORM + CRNORM + TEST_COMMENT,
				layout.getCanonicalHeaderComment(false));
		assertNull("Wrong comment for property", layout.getComment(TEST_KEY));
	}

	/**
	 * Tests if a header comment containing blanc lines is correctly detected and
	 * doesn't overflow into the property comment in the case that the header
	 * comment is already set
	 */
	@Test
	...
	/**
	 * Tests if a header comment is correctly detected when it contains blanc lines
	 * and the first property has a comment, too.
	 */
	@Test
	...
	public void testEventAdd() {
		final ConfigurationEvent event = new ConfigurationEvent(this, ConfigurationEvent.ADD_PROPERTY, TEST_KEY,
				TEST_VALUE, false);
		layout.onEvent(event);
		assertTrue("Property not stored", layout.getKeys().contains(TEST_KEY));
		assertEquals("Blanc lines before new property", 0, layout.getBlancLinesBefore(TEST_KEY));
		assertTrue("No single line property", layout.isSingleLine(TEST_KEY));
		assertEquals("Wrong separator", " = ", layout.getSeparator(TEST_KEY));
	}

	/**
	 * Tests adding a property multiple time through an event. The property should
	 * then be a multi-line property.
	 */
	@Test
	...
	public void testEventSetNonExisting() {
		final ConfigurationEvent event = new ConfigurationEvent(this, ConfigurationEvent.SET_PROPERTY, TEST_KEY,
				TEST_VALUE, false);
		layout.onEvent(event);
	...
	public void testEventDelete() {
		ConfigurationEvent event = new ConfigurationEvent(this, ConfigurationEvent.ADD_PROPERTY, TEST_KEY, TEST_VALUE,
				false);
		layout.onEvent(event);
	...
	public void testRecursiveLoadCall() throws ConfigurationException {
		...
		assertEquals(
				"Wrong comment for property", TEST_COMMENT + CRNORM + "A nested header comment." + CRNORM
						+ "With multiple lines" + CRNORM + CRNORM + "Second comment",
				layout.getCanonicalComment(TEST_KEY, false));
	}

	/**
	 * Tests whether the output of the layout object is identical to the source file
	 * (at least for simple properties files).
	 */
	@Test
	...
	public void testSave() throws ConfigurationException {
		...
		checkLayoutString("# A header comment" + CR + "# for my properties" + CR + CR + "# " + TEST_COMMENT + CR
				+ TEST_KEY + " = " + TEST_VALUE + CR + TEST_KEY + " = " + TEST_VALUE + "2" + CR + CR + CR
				+ "# AnotherComment" + CR + "AnotherProperty = AnotherValue,3rdValue" + CR);
	}
	...
	public void testSaveForceSingleLine() throws ConfigurationException {
		...
		checkLayoutString("# " + TEST_COMMENT + CR + TEST_KEY + " = " + TEST_VALUE + ';' + TEST_VALUE + "2" + CR
	...
	public void testTrimComment() {
		assertEquals("Wrong trimmed comment",
				"This is a comment" + CR + "that spans multiple" + CR + "lines in a" + CR + " complex way.",
				PropertiesConfigurationLayout.trimComment("   # This is a comment" + CR + "that spans multiple" + CR
	...
	public void testTrimCommentFalse() {
		assertEquals("Wrong trimmed comment",
				"# Comment with" + CR + " ! some mixed " + CR + "#comment" + CR + "# lines",
				PropertiesConfigurationLayout
						.trimComment("Comment with" + CR + " ! some mixed " + CR + "#comment" + CR + "lines", true));
	}
	...
	public void testGetNonExistingLayouData() {
		...
		assertEquals("Leading blanc lines", 0, layout.getBlancLinesBefore("unknown"));
	}
	...
	/**
	 * Tests saving when a comment for a non existing property is contained in the
	 * layout object. This comment should be ignored.
	 */
	@Test
	...
	public void testInitCopyModify() {
		...
		assertFalse("Blanc lines do not differ",
				layout.getBlancLinesBefore(TEST_KEY) == l2.getBlancLinesBefore(TEST_KEY));
	}
	...
	public void testSetLineSeparator() throws ConfigurationException {
		...
		checkLayoutString(
				"# Header comment" + lf + lf + lf + lf + "# " + TEST_COMMENT + lf + TEST_KEY + " = " + TEST_VALUE + lf);
	}
	...
	public void testSetLineSeparatorInComments() throws ConfigurationException {
		...
		layout.setLineSeparator(lf);
		checkLayoutString("# Header" + lf + "# comment" + lf + lf + "# " + TEST_COMMENT + lf + "# More comment" + lf
				+ TEST_KEY + " = " + TEST_VALUE + lf);
	}

	/**
	 * Tests whether a line with whitespace is handled correctly. This is related to
	 * CONFIGURATION-582.
	 */
	@Test
	...
	static class PropertiesBuilder {
		 ...
		 * Adds a comment line.
		 *
		 * @param s the comment (can be <b>null</b>, then a blanc line is added)
		 */
